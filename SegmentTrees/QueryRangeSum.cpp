// Type1: Calculate Sum in range[qs, qe];
...
void biuldSeg(int node, int l , int r){
    ...
    seg[node] =  seg[2 * node] + seg[2 * node + 1]; // merging
}
...
int query(int node, int qs, int qe, int l, int r){
    ...
    return query(2 * node, qs, qe, l, mid) + query(2 * node + 1, qs, qe, mid + 1, r);
}
